...
building this task: `brew install --cask osquery` was attempted on this
macOS dev machine and failed — the cask's installer needs an interactive
`sudo` password prompt this non-interactive environment cannot supply (see
the A-15 task report for the exact error). Every SQL query/column name below
is taken from osquery's own published schema
(https://osquery.io/schema/current, tables `listening_ports`,
`process_open_sockets`, `processes`, `file_events`, `osquery_flags`), not
empirically confirmed against a live instance — flagged in the task report
as a genuine gap, same disclosure style A-18 used for its unverified Windows
paths.
"""
...
async def _query(sql: str) -> list[dict[str, Any]]:
    ...
    try:
        returncode, stdout, stderr = await run_local_command(_OSQUERYI, "--json", sql)
    ...
    if looks_like_permission_denied(combined):
        raise OsqueryError(f"{_OSQUERYI} denied access for this query", reason="permission_denied")
    if returncode != 0:
        raise OsqueryError(f"{_OSQUERYI} exited {returncode}: {stderr.strip()}", reason="unreachable")

    try:
        rows = json.loads(stdout)
    except json.JSONDecodeError as exc:
        raise OsqueryError(f"{_OSQUERYI} returned unparseable JSON: {exc}", reason="unreachable") from exc
    if not isinstance(rows, list):
        raise OsqueryError(f"{_OSQUERYI} returned an unexpected JSON shape", reason="unreachable")
    return rows
