"""EscalationScheduler — pure logic + an injected fake sleep, no real DB
involved (that's exercised in tests/integration/test_notifications_escalation.py).
Mirrors tests/unit/test_backup_scheduler.py's fake-sleep idiom.
"""
...
from app.services.notifications.escalation import EscalationScheduler
...
async def test_loop_always_sleeps_before_the_first_run():
    ...
    scheduler = EscalationScheduler(interval_seconds=30, run_once=run_once, sleep=_fake_sleep(delays))
...
async def test_loop_reruns_on_the_same_fixed_interval():
    ...
    scheduler = EscalationScheduler(interval_seconds=10, run_once=run_once, sleep=_fake_sleep(delays))
...
async def test_start_is_idempotent_and_stop_cancels_cleanly():
    ...
    scheduler = EscalationScheduler(interval_seconds=999, run_once=run_once)
...
async def test_stop_before_start_is_a_safe_no_op():
    scheduler = EscalationScheduler(interval_seconds=30, run_once=_noop)
