...
A-30 adds `POST /consoles/logs/wazuh/syscheck` coverage below, same
monkeypatch-the-module-level-helper technique against
...
from app.services.mcp.security_connectors.wazuh import WazuhError, WazuhNotConfiguredError
...
@pytest.mark.integration
async def test_trigger_syscheck_returns_affected_agent_ids_on_success(
    client: TestClient,
    migrated_session_maker: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
):
    async def _fake_trigger(settings=None):
        return ["003"]

    monkeypatch.setattr(security_console_module, "trigger_syscheck_scan", _fake_trigger)
    headers = await _admin_headers(client, migrated_session_maker)

    response = client.post("/security/consoles/logs/wazuh/syscheck", headers=headers)

    assert response.status_code == 200
    assert response.json() == {"affected_items": ["003"]}


@pytest.mark.integration
async def test_trigger_syscheck_reports_not_configured_as_503_not_500(
    client: TestClient,
    migrated_session_maker: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
):
    async def _fake_trigger(settings=None):
        raise WazuhNotConfiguredError("not set up")
    ...
    assert response.json()["detail"] == {"error": "wazuh_not_configured"}


@pytest.mark.integration
async def test_trigger_syscheck_maps_wazuh_error_reason_into_the_error_code(
    client: TestClient,
    migrated_session_maker: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
):
    async def _fake_trigger(settings=None):
        raise WazuhError("nope", reason="unauthorized")
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == {"error": "wazuh_unauthorized"}


@pytest.mark.integration
async def test_trigger_syscheck_requires_authentication(client: TestClient):
    response = client.post("/security/consoles/logs/wazuh/syscheck")
