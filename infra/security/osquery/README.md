...
> **2026-07-18 (A-24, готово для macOS/Linux; Windows — написано, ждёт реального прогона
> CI):** ручная установка ниже нужна только для **Docker/venv-режима разработки**. Нативные
> установщики (macOS/Linux/Windows, A-20…A-22) вшивают бинарник osquery прямо в установку —
> конечному пользователю ничего из этого файла делать не придётся. macOS: подтверждено живьём
> на этой машине (собранный `.app` с вшитым `osqueryi` показал `connector.status: "ok"` на
> консолях «Сеть»/«Вирусная активность» без системного `osqueryi` на `PATH`). Linux: подтверждено
> внутри тестового systemd-контейнера (`packaging/linux/test/Dockerfile.systemd`) — та же честная
> оговорка, что и для остальной части Linux-установщика (контейнер — не хост конечного
> пользователя). Windows: код написан и добавлен в `.github/workflows/windows-build.yml`, но
> реальный прогон возможен только на настоящем `windows-latest`-раннере — не проверено в этой
> сессии (нет доступа к GitHub Actions). См. `packaging/{macos,linux,windows}/README.md`'s
> собственные разделы про osquery и
> [план-спецификация-фаза-0-контур-безопасности-100-2026-07-18.md](../../../docs/план-спецификация-фаза-0-контур-безопасности-100-2026-07-18.md).
